# graphics.py
# Coded by George H. Chen (georgehc@mit.edu) -- updated 10/12/2012 4:55pm
...
class playback_positions(tk.Tk):
    ...
    def __init__(self, true_positions, observed_positions,
        ...
        for column in range(GRID_WIDTH):
            for row in range(GRID_HEIGHT):
                ...
                y1 = row * CELL_HEIGHT + \
                     3 * (GRID_HEIGHT * CELL_HEIGHT + PADDING)
                y2 = y1 + CELL_HEIGHT
                for direction in ['up','down','left','right', 'stay']:
                    trapezoid_coordinates = self.get_trapezoid_coords(x1,x2,y1,y2, direction)
                    tags = ('heat_map_%d_%d_%s'%(row,column,direction),
                            'heat_map')
                    self.heat_map[row, column, direction] = \
                        self.canvas.create_polygon(trapezoid_coordinates,
                                                   fill='black',
                                                   tags=tags,
                                                   outline='gray11')
    ...
    def redraw(self, true_positions, observed_positions, estimated_positions,
        ...
        if len(true_positions) == 0:
            ...
            self.canvas.itemconfig('rect_bottom', fill='black',
                                   outline='gray11')
            self.canvas.itemconfig('heat_map', fill='black',
                                   outline='gray11')
    ...
    def color_heatmap_grid(self, marginals):
        ...
        for state, prob in marginals.iteritems():
        ...
        for x in range(GRID_WIDTH):
            for y in range(GRID_HEIGHT):
                ...
                self.canvas.itemconfigure(self.rect_bottom[y, x], 
                    fill='#%02x%02x%02x' % (color, 0, 0)) 
